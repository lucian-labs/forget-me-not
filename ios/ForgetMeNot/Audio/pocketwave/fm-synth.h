    ...
    for (int i = 0; i < frames; i++) {
        ...
        for (int v = 0; v < FM_MAX_VOICES; v++) {
            ...
            case FM_ENV_SUSTAIN:
                ...
                break;
            case FM_ENV_RELEASE: {
                /* Exponential release — one-pole approach to 0 with time
                 * constant rel/3 (so ~95% decay at t = rel, matching the
                 * subjective length of the previous linear curve). The
                 * asymptotic tail eliminates the click from a hard cut at
                 * t = 1 that linear release produced. */
                if (rel < 0.0001f) {
                    dead = 1; env = 0;
                } else {
                    vc->env_level *= expf(-3.0f * dt / rel);
                    env = vc->env_level;
                    if (env < 0.0005f) { dead = 1; env = 0; }
                }
            } break;
            }
