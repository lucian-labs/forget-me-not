...
typedef struct {
    ...
    /* Microtonal quantize — per-slot */
    int              quantize_mode;       /* 0=snap (default), 1=microtonal, 2=chromatic */
    float            micro_amount;        /* 0.0–1.0 — depth of cents offset for off-scale notes */
    float            micro_cents_active;  /* current microtonal cents — set on off-scale note-on, cleared on release */

    /* Tonal flair — fire a 4-note voicing on note-on while a gate CC is held */
    int              flair_mode;          /* 0=off, 1=on */
    int              flair_cc;            /* MIDI CC# that gates flair (default 64 = sustain) */
    int              flair_held;          /* 1 while gate CC ≥ 64 */
